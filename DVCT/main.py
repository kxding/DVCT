...
def save_progress(tokenizer, text_encoder, accelerator, args, global_step):
    ...
    for placeholder_token in tokenizer.token_map:
        backward_prompt = [placeholder_token]
        ...
        _text_encoder = accelerator.unwrap_model(text_encoder)
        text_embedding = _text_encoder(text_input.input_ids.to(device))[0]
        torch.save(text_embedding, os.path.join(args.output_dir, f'{global_step}.bin'))
...
def parse_args():
    ...
    parser.add_argument(
        "--guidance_scale_train_src",
        type=float,
        default=7.5,
        help="Guidance scale for src training.",
    )

    parser.add_argument(
        "--guidance_scale_train_ref",
        type=float,
        default=7.5,
        help="Guidance scale for ref training.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.9,
        help="the weight of consine loss",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="use controller or not",
    )
    parser.add_argument(
        "--use_prev_embeddings",
        action="store_true",
        default=False,
        help="use previos embedding or not",
    )
    parser.add_argument(
        "--only_train",
        action="store_true",
        default=False,
        help="only training embeddings",
    )
    parser.add_argument(
        "--use_dds",
        action="store_true",
        default=False,
        help="use dds loss or not",
    )
    parser.add_argument(
        "--src_embedding_path",
        type=str,
        default="./output/",
        help="trained src_embedding dir",
    )
    parser.add_argument(
        "--report_to",
        type=str,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Whether or not to use direct inversion.",
    )
...
def multi_concept_inversion():
    args = parse_args()

    # print(f"use ref cfg: {args.use_ref_cfg}")
    # if args.use_ref_cfg:
    #     print("    Using ref classifier free guidance...")
    #     print(f"    guidance_scale_train_ref: {args.guidance_scale_train_ref}")
    
    # src_embedding_list, src_reversed_latent_list, src_file_name = train_src.train_src_embedding(
    #     pretrained_model_name_or_path=args.pretrained_model_name_or_path,
    #     src_image_path=args.content_image_dir,
    #     forward_prompt="",
    #     guidance_scale=args.guidance_scale_train_src,
    # )

    # obtain the concept folder name form its path
    concept_folder_name = os.path.split(os.path.normpath(args.concept_image_dir))[-1] or \
                          os.path.splitdrive(os.path.normpath(args.concept_image_dir))[-1]
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(text_encoder):
                ...
                ref_pred = unet(noisy_latents, timesteps, encoder_hidden_states).sample
                ...
                # loss = F.mse_loss(cfg_pred.float(), target.float(), reduction="mean")
                loss = F.mse_loss(ref_pred.float(), target.float(), reduction="none")
                loss = loss.mean()

                if args.use_l1 or args.use_l1_pixel:
                    rec_latent = noise_scheduler.get_original(ref_pred, noisy_latents, timesteps)
...
if __name__ == "__main__":
    ...
    args = parse_args()
    if args.use_prev_embeddings != True:
        multi_concept_inversion()
    # else:
    #     print("src_embedding: " + str(args.src_embedding_path))
    #     src_embeds_dict_name = "_%s.bin" % (str(args.max_train_steps))
    #     src_embeds_dict_path = os.path.join(args.src_embedding_path, src_embeds_dict_name)
    #     src_embedding_list = torch.load(src_embeds_dict_path)
    #     src_reversed_latent_list = torch.load(os.path.join(args.src_embedding_path, "src_reversed_latent_list.bin"))
    #     args.output_dir = args.src_embedding_path
    ...
    # src_file_name = get_image_file(args.content_image_dir).split('.')[0]
    # os.makedirs(args.output_image_path, exist_ok=True)
    if not args.only_train:
        if not args.use_dds:
            gen_image.gen_image(
                pretrained_model_name_or_path=args.pretrained_model_name_or_path,
                output_img_path_name=args.output_image_path + "/" + ref_file_name + "_" + src_file_name + "_" + str(args.guidance_scale_train_ref) + "_ctrl-" + str(not args.no_controller) + "_c-" + str(args.cross_attention_injection_ratio) + "_s-" + str(args.self_attention_injection_ratio) + "_" + time_smp + ".png",
                backward_prompts=["", "<s2>"],
                model_id=args.output_dir,
                train_step=args.max_train_steps,
                prompt_name_backward="s2",
                src_reversed_latent_list=src_reversed_latent_list,
                src_embedding_list=src_embedding_list,
            ...
            gen_image.gen_image_dds(
                pretrained_model_name_or_path=args.pretrained_model_name_or_path,
                output_img_path_name=args.output_image_path + "/" + ref_file_name + "_" + src_file_name + "_" + str(args.guidance_scale_train_ref) + "_ctrl-" + str(not args.no_controller) + "_DI-" + str(args.use_direct_inversion) + "_c-" + str(args.cross_attention_injection_ratio) + "_s-" + str(args.self_attention_injection_ratio) + "_" + time_smp + ".png",
                backward_prompts=["", "<s2>"],
                model_id=args.output_dir,
                train_step=args.max_train_steps,
                prompt_name_backward="s2",
